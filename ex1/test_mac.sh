...
i=1
while true; do
  if [ -e "inputs/$PROG.$i.in" ]; then
    out=$(mktemp -t "$PROG")
    timeout 1s java "$PROG" < "inputs/$PROG.$i.in" | head -c 50MB > "$out"
    status="$?"
    if [ "$status" -ne 0 ]; then
      echo "$PROG: returned non-zero status $status for test case $i"
      num_failed=$((num_failed + 1))
    else 
      if [ -e "$out" ]; then
        if [ "$(diff -bB "$out" "outputs/$PROG.$i.out" | wc -l)" -ne 0 ]; then
          echo "test $i: failed"
          num_failed=$((num_failed + 1))
        else
          echo "test $i: passed"
