...
class BankSimulation extends Simulation {
  ...
  public BankSimulation(Scanner sc) {
    ...
    int maxCounterSize = sc.nextInt();
    int queueSize = sc.nextInt();
    Bank bank = new Bank(numOfCounters, queueSize, maxCounterSize);
    ...
    while (sc.hasNextDouble()) {
      ...
      Task task = taskType == 1 ? new TaskWithdrawal(amount) : new TaskDeposit(amount);
      Customer newCustomer = new Customer(id, serviceTime, task, amount);
